...
void test_basic() {
    ...
    check_distance(just_get_dist_gpu(template, 3, 3, template, 3), 0);
    check_distance(just_get_dist_gpu(translated, 5, 4, template, 3), 1);
    check_distance(just_get_dist_gpu(rotated, 3, 3, template, 3), 2);
    check_distance(just_get_dist_gpu(flip_x_axis, 3, 3, template, 3), 3);
    check_distance(just_get_dist_gpu(flip_y_axis, 3, 3, template, 3), 4);
}
...
void test_bmp(char *img_name) {
    ...
    for(int i = 0; i < num_templates; i++) {
        distances[i] = just_get_dist_gpu(img.data, img.width, img.height, templates[i], TEMPLATE_WIDTH);
    }
